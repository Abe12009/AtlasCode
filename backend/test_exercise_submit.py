...
async def test():
    ...
    async with AsyncClient(app=app, base_url="http://test") as client:
        ...
        # Get exercise detail
        response = await client.get("/exercises/1", headers=headers)
        print("Exercise detail: " + str(response.status_code))
        if response.status_code != 200:
            print("Error: " + response.text)
        else:
            data = response.json()
            print("Exercise: " + str(data))
            ...
            print("Submit: " + str(response.status_code))
            if response.status_code != 200:
                print("Error: " + response.text)
            else:
                result = response.json()
                print("Submit result: " + str(result))
